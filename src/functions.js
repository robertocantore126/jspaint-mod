...
/**
 * Loads a layer or set as a selection: the node's pixels become a floating selection that can be
 * moved, copied, or cut out, like "Select All" but limited to one part of the document.
 *
 * Nothing is taken out of the document here -- the selection floats on top, and the pixels stay put
 * until the selection is melded (which is what moving it or switching tools does).
 *
 * The selection is a rectangle covering the bounding box of the node's visible content.
 *
 * @param {number} id
 * @returns {boolean} whether there was anything to select
 */
function load_node_as_selection(id) {
	const snapshot = document_model.to_node_canvas(id);
	if (!snapshot) {
		$status_text.text(localize("There's nothing to select in this layer or set."));
		setTimeout(() => $status_text.default(), 2500);
		return false;
	}
	deselect();
	select_tool(get_tool_by_id(TOOL_SELECT));
	...
	undoable({
		name: localize("Load Selection"),
		icon: get_icon_for_tool(get_tool_by_id(TOOL_SELECT)),
		soft: true,
	}, () => {
		selection = new OnCanvasSelection(snapshot.x, snapshot.y, snapshot.canvas.width, snapshot.canvas.height, snapshot.canvas);
	});
	return true;
}
